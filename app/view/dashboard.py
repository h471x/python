...
def dashboard_page(dashboard, widget, content):
    ...
    for j in range(2):
        home_container.grid_columnconfigure(j, weight=1)
...
def settings_page(dashboard, widget, content):
    ...
    users_list.pack(
        side="left",  # Pack to the left side
        padx=(13, 0),
        pady=15,
        expand=True
    )

    users_list2 = widget.new_dropdown(
        top_container,
        [user[0] for user in users],
        125, 30, "center"
    )
    users_list2.pack(
        side="right",  # Pack to the right side
        padx=(0, 13),
        pady=15,
        expand=True
    )
